    ...
    public int getColumn() {
        ...
        for (column = 0; column < board.length; column++) {
            ...
            if (row != -1 && checkXInARow(column, row, 4, thisPlayer, board)) {
                if (debug) {
                    System.out.println("Found something that makes me win: "+row+"-" + (column + 1));
                }
                return column;
            }
        }
        ...
        for (column = 0; column < board.length; column++) {
            ...
            if (row != -1 && checkXInARow(column, row, 4, enemyPlayer, board)) {
                if (debug) {
                    System.out.println("Found something that makes enemy win:"+row+"-"  + (column + 1)); // debug
                }
                return column;
            }
        }
        ...
        for (column = 0; column < 7; column++) {
            ...
            if (row != -1 && checkXInARow(column, row, 3, enemyPlayer, board)) {
                if (debug) {
                    System.out.println("Found something good: "+row+"-"+ (column + 1)); // debug                                       
                }
            ...
            if (row != -1 && checkXInARow(column, row, 2, enemyPlayer, board)) {
                if (debug) {
                    System.out.println("Found something (maybe) valuable:"+row+"-"+ (column + 1)); // debug
                }
                possibleSolutions.add(column);
            }
        }
       ...
       // if there are only bad ideas, choose a random valid column:
        if (veryBadIdeas.size() == 7) {
           do {
               ...
               column = grn.nextInt(3) + grn.nextInt(3);
